...
#error Wont include multuple solution headers
#endif
...
public:
  int rob(std::vector<int> &nums) {
    ...
    bestAt[0] = nums[0];
    bestAt[1] = nums[1];
    // Initialize with 0 or nums[0] since paths only begin to diverge after
    // index 2 The only way to get to 2: 0, but there are 2 ways to get to 3:
    // 0->3, 1->3
    ...
    // element, and one has to end on the penultimate element (see
    // notes/0198.org)
